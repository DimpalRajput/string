# 9Chapter: File Handling in Python
...
# # Variables store data only while a program is running. Once the program stops, #  the values stored in memory are usually lost.
#  Files provide a way to store data permanently so that it can be accessed later.
# # Python provides built-in tools for creating, opening, reading, writing, # updating, and deleting files.
# It also includes specialized modules for working
#  with structured file formats such as CSV and JSON. # # ---
# # ## 3. What Is a File?
# # A file is a collection of data stored on a storage device such as a hard drive, # # solid-state drive, USB drive, or cloud storage system.
...
# #    photo.jpg
# #    ```
# # 2. **File extension** : The part after the final dot in the file name. # It often indicates the type of content stored in the file.
# #    * `.txt` — plain text
# #    * `.csv` — comma-separated values # #    * `.json` — JSON data
# #    * `.py` — Python source code # #    * `.jpg` — image
# #    * `.pdf` — PDF document# # A file extension helps users and programs understand
...
# # * `mode`: The operation to perform, such as reading or writing. # # * `encoding`: The character encoding used for text files.
# # Example:
# # ```python
file = open("STRING/file handling.py", "r", encoding="utf-8") 
content =file.read()
print(content)
print(file.closed)
file.close()
...
# ---
# ## 6. File Modes
# The file mode tells Python how the file will be used.
# | Mode  | Meaning|
# | ----- | ------------------------------------------------------ | # | `"r"` | Opens a file for reading|
# | `"w"` | Opens a file for writing and replaces existing content |
# | `"a"` | Opens a file for appending|
# | `"x"` | Creates a new file and fails if it already exists      |# | `"t"` | Opens a file in text mode
# | `"b"` | Opens a file in binary mode|
# | `"+"` | Opens a file for both reading and writing              | # ### Common combinations
# | Mode   | Description
# | ------ | ------------------------------------------------ ||
# | `"rt"` | Read a text file
...
# | `"r+"` | Read and write without deleting existing content | # | `"w+"` | Read and write while replacing existing content ||
# | `"a+"` | Read and append
...
# ```python
# file = open("notet"s.txt", "r", encoding="utf-8") # content = file.read()
# file.close()
# ```
# Closing a file:
# * Releases system resources.
# * Ensures buffered data is written properly. # * Prevents unnecessary file locks.
# * Reduces the chance of data corruption.
# A file can be checked using the `closed` attribute:
# ```python
# file = open("notes.txt", "r", encoding="utf-8") # print(file.closed) # False# file.close() # file closing
...
# True
# ```
# However, manually calling `close()` is not the preferred approach because an error #  may occur before Python reaches the closing statement.
# ---
...
# ```python
with open("STRING/file handling.py", "r", encoding="utf-8") as file:
 content = file.read()
print(content)
print(file.closed)
# print(file.closed)``
...
# # Part I: Reading Text Files# ## 9. Reading an Entire File with `read()`
...
# Python code:
# ```python
# with open("message.txt", "r", encoding="utf-8") as file: #     content = file.read()
# print(content)
# ```
# Output:
# ```text
...
# ```python
with open("STRING/file handling.py", "r", encoding="utf-8") as file: first_ten = file.read(10)
print(first_ten)
# # ```
# This reads at most ten characters.# Reading an entire file is convenient, but it may use too much memory when the file is very large.
# ---
# ## 10. Reading One Line with `readline()`
# The `readline()` method reads one line at a time.
# ```python
with open("/file handling", "r", encoding="utf-8") as file:
...
# ```python
# with open("/Users/pinkisharma/Desktop/python/session1/tuples.py", "r", encoding="utf-8") as file:
...
# ```python
with open("python/tuples.py", "r", encoding="utf-8") as file:
 lines = file.readlines()
print(lines)
# # ```
# Each item in the list represents one line.# ```python
for line in lines:
 print(line.strip())
# ```
...
with open("python/tuples.py", "r", encoding="utf-8")as file:
     for line in file:
         print(line.strip())
# ```
# Python reads each line when it is needed rather than loading the entire file at once. # This approach is recommended for large files.
...
# # Part II: Writing Text Files
# ## 14. Writing with `"w"` Mode
...
with open("STRING/message.txt", "w", encoding="utf-8") as file:
...
# ```
# ### Important warning
# Opening an existing file in `"w"` mode removes its previous content immediately. # ---# ## 15. Writing Multiple Lines
# The `write()` method does not automatically add a newline.
# ```python
 with open("file handling/message.txt", "w", encoding="utf-8") as file:
...
# ```
# `writelines()` does not automatically insert newline characters. They must be included in the strings.
...
# # Part III: File Positions # ## 19. The File Pointer# When a file is opened, Python maintains a current position called the **file pointer** or
# **cursor**.
# The pointer indicates where the next read or write operation will occur.
# ```python
with open("python/STRING/file handling.py", "r", encoding="utf-8") as file:
     print(file.read(6))
     print(file.read(30))
# # # ```
# The second `read(5)` starts from the position where the first operation ended.
# ---
# ## 20. Finding the Current Position with `tell()`
...
with open("STRING/file handling.py", "r", encoding="utf-8") as file:
     ...
     print(file.tell())
# ```
# In text mode, the returned value should be treated as an opaque position value rather # than always assuming it is a simple character number.
# ---
# ## 21. Changing the Position with `seek()`
...
with open("STRING/file handling.py", "r", encoding="utf-8") as file:     
    ...
    print(second_read)# `file.seek(0)` moves the pointer back to the beginning.
